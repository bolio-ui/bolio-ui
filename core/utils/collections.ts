...
export const isBolioUIElement = (el?: HTMLElement): boolean => {
  if (!el) return false
  if (el?.dataset && el?.dataset['bolio-ui']) return true
  el.attributes.getNamedItem('data-bolio-ui')
  return !!el.attributes.getNamedItem('data-bolio-ui')
}
