...
import { useToasts, Button, Spacer, Link } from 'core'

export const meta = {
  title: 'useToast',
  description: 'Display an important message globally.',
  sidebar: 'hooks',
  group: 'Hooks',
  index: 13
}
...
() => {
  const { setToast } = useToasts()
  const click = () => setToast({
    text: <p>Click the <Link target="_blank" color block href="https://github.com/bolio-ui/bolio-ui">URL</Link> to learn more</p>,
    delay: 4000,
  })
  return <Button scale={2/3} auto onClick={click}>Show Toast</Button>
}
...
<Attributes edit="/content/docs/hooks/use-toast.mdx">
