...
import { Playground, PlaygroundTitle, Attributes } from 'src/components'
import { useMediaQuery, Link, Code, Spacer } from 'core'
...
CSS media query hooks, it is implemented through the `MediaQuery` API.
If you just want to build the layout, try <Link color href="/docs/components/grid">Grid Component</Link>.
...
<PlaygroundTitle
  ...
  desc="Override the default breakpoints of the `@bolio-ui/core`."
/>

```tsx
const breakpoints: BolioUIThemesBreakpoints = {
  xs: { min: '0', max: '650px' },
...
const App = () => {
  ...
  return (
    <BolioProvider themes={[myTheme]} themeType="myTheme">
      <CssBaseline />
      <AppComponent />
    </BolioProvider>
  )
...
<Attributes edit="/content/docs/hooks/use-media-query.mdx">
