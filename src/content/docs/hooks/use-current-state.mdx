...
import { useCurrentState, Link, Button, Spacer } from 'core'

export const meta = {
  title: 'useCurrentState',
  description:
    'Get the latest value in functional component. By default, `state` in a React function component can only get the value at render time, not the latest.',
  sidebar: 'hooks',
  group: 'Hooks',
  index: 5
}
...
<Attributes edit="/content/docs/hooks/use-current-state.mdx">
