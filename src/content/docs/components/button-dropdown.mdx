import Docs from 'src/templates/Docs'
import { ButtonDropdown, Grid } from 'core'
import { Playground, Attributes } from 'src/components'
import { Plus } from '@bolio-ui/icons'

export const meta = {
  title: 'Button Dropdown',
  description: 'Display related but alternate actions for a button.',
  sidebar: 'components',
  group: 'Components',
  index: 5
}

## Button Dropdown

Display related but alternate actions for a button.

<Playground
  title="General"
  desc="The basic `ButtonDropdown` usage."
  scope={{ ButtonDropdown, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <ButtonDropdown>
      <ButtonDropdown.Item main>Default Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Loading"
  desc="Show friendly load indicator."
  scope={{ ButtonDropdown, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <ButtonDropdown loading>
      <ButtonDropdown.Item main>Default Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Disabled"
  desc="Disable all buttons."
  scope={{ ButtonDropdown, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <ButtonDropdown disabled>
      <ButtonDropdown.Item main>Default Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Types"
  desc="You can change the color of the `ButtonDropdown` with the property `type`."
  scope={{ ButtonDropdown, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid xs={12} sm={6} md={4}>
    <ButtonDropdown type="primary">
      <ButtonDropdown.Item main>Primary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
  <Grid xs={12} sm={6} md={4}>
    <ButtonDropdown type="secondary">
      <ButtonDropdown.Item main>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
  <Grid xs={12} sm={6} md={4}>
    <ButtonDropdown type="success">
      <ButtonDropdown.Item main>Success Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
  <Grid xs={12} sm={6} md={4}>
    <ButtonDropdown type="warning">
      <ButtonDropdown.Item main>Warning Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
  <Grid xs={12} sm={6} md={4}>
    <ButtonDropdown type="error">
      <ButtonDropdown.Item main>Error Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
  <Grid xs={12} sm={6} md={4}>
    <ButtonDropdown type="info">
      <ButtonDropdown.Item main>Info Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Secondary Button</ButtonDropdown.Item>
      <ButtonDropdown.Item>Tertiary Button</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Multiple Type"
  desc="Specify the status of a button individually."
  scope={{ ButtonDropdown, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <ButtonDropdown scale={2 / 3} auto>
      <ButtonDropdown.Item main>Check</ButtonDropdown.Item>
      <ButtonDropdown.Item>Block</ButtonDropdown.Item>
      <ButtonDropdown.Item>Lock</ButtonDropdown.Item>
      <ButtonDropdown.Item type="error">Destroy</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Custom Right Icon"
  desc="The `color` and `size` of the right icon will be set automatically."
  scope={{ ButtonDropdown, Grid, Plus }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <ButtonDropdown scale={2 / 3} auto icon={<Plus />}>
      <ButtonDropdown.Item main>Check User</ButtonDropdown.Item>
      <ButtonDropdown.Item>Block User</ButtonDropdown.Item>
      <ButtonDropdown.Item>Lock User</ButtonDropdown.Item>
      <ButtonDropdown.Item type="error">Destroy User</ButtonDropdown.Item>
    </ButtonDropdown>
  </Grid>
</Grid.Container>
`}
/>

<Attributes edit="/content/docs/components/button-dropdown.mdx">
<Attributes.Title>ButtonDropdown.Props</Attributes.Title>

| Attribute    | Description               | Type                  | Accepted values                             | Default              |
| ------------ | ------------------------- | --------------------- | ------------------------------------------- | -------------------- |
| **type**     | button type               | `ButtonDropdownTypes` | [ButtonDropdownTypes](#buttondropdowntypes) | `default`            |
| **loading**  | display loading indicator | `boolean`             | -                                           | `false`              |
| **auto**     | autoscale width           | `boolean`             | -                                           | `false`              |
| **disabled** | disable button            | `boolean`             | -                                           | `false`              |
| **icon**     | custom right icon         | `ReactNode`           | -                                           | `ButtonDropdownIcon` |
| ...          | native props              | `HTMLAttributes`      | `'autoFocus', 'name', 'className', ...`     | -                    |

<Attributes.Title>ButtonDropdown.Item.Props</Attributes.Title>

| Attribute   | Description     | Type                   | Accepted values                             | Default   |
| ----------- | --------------- | ---------------------- | ------------------------------------------- | --------- |
| **type**    | button type     | `ButtonDropdownTypes`  | [ButtonDropdownTypes](#buttondropdowntypes) | `default` |
| **main**    | the main action | `boolean`              | -                                           | `false`   |
| **onClick** | event handler   | `MouseEventHandler`    | -                                           | -         |
| ...         | native props    | `ButtonHTMLAttributes` | `'id', 'name', 'className', ...`            | -         |

<Attributes.Title>ButtonDropdownTypes</Attributes.Title>

```ts
type ButtonDropdownTypes =
  | 'default'
  | 'primary'
  | 'secondary'
  | 'success'
  | 'warning'
  | 'error'
  | 'info'
```

</Attributes>

export default Docs.withMeta(meta)
