...
import { Popover, Grid, Spacer, Link } from 'core'
import { useState } from 'react'

export const meta = {
  title: 'Popover',
  description: 'The floating box popped by clicking or hovering.',
  sidebar: 'components',
  group: 'Components',
  index: 28
}
...
<Playground
  desc="The basic `Popover` usage."
  scope={{ Popover, Grid, Spacer, Link }}
  code={`
() => {
  const content = () => (
...
<Playground
  ...
  scope={{ Popover, Spacer, Grid, Link, useState }}
  code={`
() => {
...
<Attributes edit="/content/docs/components/popover.mdx">
