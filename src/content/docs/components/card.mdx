import Docs from 'src/templates/Docs'
import { Playground, Attributes } from 'src/components'
import { Card, Link, Image, Text, Divider, Code, Grid } from 'core'

export const meta = {
  title: 'Card',
  description: 'A common container component.',
  sidebar: 'components',
  group: 'Components',
  index: 8
}

## Card

A common container component.

<Playground
  title="General"
  desc="The basic `Card` usage."
  scope={{ Card, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <Card>A default card.</Card>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Hoverable"
  desc="You can apply a fancy hover animation with the `hoverable` prop."
  scope={{ Card, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid xs={12} md={4}>
    <Card hoverable>A hoverable default card.</Card>
  </Grid>
  <Grid xs={12} md={4}>
    <Card hoverable type="primary">A hoverable primary card.</Card>
  </Grid>
  <Grid xs={12} md={4}>
    <Card hoverable type="secondary">A hoverable secondary card.</Card>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Bordered"
  desc="You can apply a fancy hover animation with the `bordered` prop."
  scope={{ Card, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid xs={12} md={4}>
    <Card bordered>A bordered default card.</Card>
  </Grid>
  <Grid xs={12} md={4}>
    <Card bordered type="primary">A bordered primary card.</Card>
  </Grid>
  <Grid xs={12} md={4}>
    <Card bordered type="secondary">A bordered secondary card.</Card>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Shadow"
  desc="You can apply shadow with the `shadow` prop."
  scope={{ Card, Grid, Text }}
  code={`
<Grid.Container gap={2}>
  <Grid xs={12} md={4}>
    <Card shadow>A shadow default card.</Card>
  </Grid>
  <Grid xs={12} md={4}>
    <Card shadow type="primary">A shadow primary card.</Card>
  </Grid>
  <Grid xs={12} md={4}>
    <Card shadow type="secondary">A shadow secondary card.</Card>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Types"
  desc="Show different states with colors."
  scope={{ Card, Grid, Text }}
  code={`
() => {
  const types = ['default', 'primary', 'secondary', 'success', 'warning', 'error', 'info', 'dark', 'lite']
  return (
    <Grid.Container gap={2}>
      {types.map((type) => (
        <Grid xs={6} md={4} key={type}>
          <Card type={type} width="100%">
            <Text h4 my={0} style={{ textTransform: 'uppercase' }}>
              {type}
            </Text>
            <Text>{type}</Text>
          </Card>
        </Grid>
      ))}
    </Grid.Container>
  )
}
`}
/>

<Playground
  title="With Footer"
  desc="You can use the `Card.Footer` component to add actions, details or another information to the `Card`."
  scope={{ Card, Link, Grid, Text }}
  code={`
<Grid.Container gap={2}>
  <Grid xs={12} md={6} justify="center">
    <Card width="100%">
      <Text h4 my={0}>
        Bolio UI
      </Text>
      <Text>More creative and fun with amazing features for React.</Text>
      <Card.Footer>
        <Link
          color
          target="_blank"
          href="https://github.com/bolio-ui/bolio-ui"
        >
          Visit source code on GitHub.
        </Link>
      </Card.Footer>
    </Card>
  </Grid>
  <Grid xs={12} md={6} justify="center">
    <Card width="100%" type="dark">
      <Text h4 my={0}>
        Bolio UI
      </Text>
      <Text>More creative and fun with amazing features for React.</Text>
      <Card.Footer>
        <Link target="_blank" href="https://github.com/bolio-ui/bolio-ui">
          Visit source code on GitHub.
        </Link>
      </Card.Footer>
    </Card>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="With Image"
  desc="You can use the `Image` component to add an image to the `Card`."
  scope={{ Card, Link, Text, Image, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <Card width="350px">
      <Image
        src="https://images.pexels.com/photos/1779487/pexels-photo-1779487.jpeg"
        width="350px"
        draggable={false}
      />
      <Text h4 mb={0} mt={0}>
        Bolio UI
      </Text>
      <Text type="primary" small>
        More creative and fun with amazing features for React.
      </Text>
      <Card.Footer>
        <Link
          block
          target="_blank"
          href="https://github.com/bolio-ui/bolio-ui"
        >
          Visit source code on GitHub.
        </Link>
      </Card.Footer>
    </Card>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="With Divider"
  desc="Show different types of content using `Divider`."
  scope={{ Card, Text, Divider, Code, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <Card width="400px">
      <Card.Content>
        <Text b my={0}>
          A Simple Component
        </Text>
      </Card.Content>
      <Divider h="1px" my={0} />
      <Card.Content>
        <Text>
          React components implement a <Code>render()</Code> method that takes
          input data and returns what to display. This example uses an
          XML-like syntax called JSX. Input data that is passed into the
          component can be accessed by <Code>render()</Code> via{' '}
          <Code>this.props</Code>.
        </Text>
      </Card.Content>
    </Card>
  </Grid>
</Grid.Container>
`}
/>

<Attributes edit="/content/docs/components/card.mdx">
<Attributes.Title>Card.Props</Attributes.Title>

| Attribute     | Description         | Type                  | Accepted values         | Default   |
| ------------- | ------------------- | --------------------- | ----------------------- | --------- |
| **hoverable** | add effect on hover                        | `boolean`              | -                       | `false`   |
| **shadow**    | show shadow                                | `boolean`              | -                       | `false`   |
| **type**      | card type                                  | [CardType](#cardtype)  | -                       | `default` |
| **filled**    | filled variant, solid background           | `boolean`              | -                       | `false`   |
| **ghost**     | outline variant, always shows a border     | `boolean`              | -                       | `false`   |
| **subtle**    | subtle variant, no background              | `boolean`              | -                       | `false`   |
| ...           | native props                               | `HTMLAttributes`       | `'id', className', ...` | -         |

<Attributes.Title alias="Card.Body">Card.Content.Props</Attributes.Title>

| Attribute | Description  | Type             | Accepted values         | Default |
| --------- | ------------ | ---------------- | ----------------------- | ------- |
| ...       | native props | `HTMLAttributes` | `'id', className', ...` | -       |

<Attributes.Title alias="Card.Actions">Card.Footer.Props</Attributes.Title>

| Attribute             | Description                   | Type             | Accepted values         | Default |
| --------------------- | ----------------------------- | ---------------- | ----------------------- | ------- |
| **disableAutoMargin** | cancel automatic margin value | `boolean`        | -                       | `false` |
| ...                   | native props                  | `HTMLAttributes` | `'id', className', ...` | -       |

<Attributes.Title>CardType</Attributes.Title>

```ts
type CardType =
  | 'default'
  | 'primary'
  | 'secondary'
  | 'success'
  | 'warning'
  | 'error'
  | 'info'
  | 'dark'
  | 'lite'
```

</Attributes>

export default Docs.withMeta(meta)
