...
<Playground
  title="Rounded"
  desc="You can completely round the corners of any type of `Input` with the `rounded` property."
  scope={{ Input, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
...
<Playground
  title="Password"
  desc="`Input` component with a show/hide password functionality, Important: You have to use the `Input.Password` component."
  scope={{ Input, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
...
<Attributes edit="/content/docs/components/input.mdx">
