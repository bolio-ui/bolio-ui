import Docs from 'src/templates/Docs'
import { Playground, Attributes } from 'src/components'
import { Tag, Grid } from 'core'

export const meta = {
  title: 'Tag',
  description: 'Display a unique keyword, optionally as a part of a set.',
  sidebar: 'components',
  group: 'Components',
  index: 37
}

## Tag

Display a unique keyword, optionally as a part of a set.

<Playground
  desc="The basic `Tag` usage."
  scope={{ Tag, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <Tag>Status: Unstable</Tag>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Types"
  desc="Show different states with colors."
  scope={{ Tag, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <Tag>Default</Tag>
  </Grid>
  <Grid>
    <Tag type="primary">Primary</Tag>
  </Grid>
  <Grid>
    <Tag type="secondary">Secondary</Tag>
  </Grid>
  <Grid>
    <Tag type="success">Success</Tag>
  </Grid>
  <Grid>
    <Tag type="warning">Warning</Tag>
  </Grid>
  <Grid>
    <Tag type="error">Error</Tag>
  </Grid>
  <Grid>
    <Tag type="dark">Dark</Tag>
  </Grid>
  <Grid>
    <Tag type="lite">Lite</Tag>
  </Grid>
</Grid.Container>
`}
/>

<Playground
  title="Invert"
  desc="Invert color and background."
  scope={{ Tag, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
    <Tag invert>Default</Tag>
  </Grid>
  <Grid>
    <Tag type="primary" invert>Primary</Tag>
  </Grid>
  <Grid>
    <Tag type="secondary" invert>Secondary</Tag>
  </Grid>
  <Grid>
    <Tag type="success" invert>Success</Tag>
  </Grid>
  <Grid>
    <Tag type="warning" invert>Warning</Tag>
  </Grid>
  <Grid>
    <Tag type="error" invert>Error</Tag>
  </Grid>
  <Grid>
    <Tag type="dark" invert>Dark</Tag>
  </Grid>
  <Grid>
    <Tag type="lite" invert>Lite</Tag>
  </Grid>
</Grid.Container>
`}
/>

<Attributes edit="/content/docs/components/tag.mdx">
<Attributes.Title>Tag.Props</Attributes.Title>

| Attribute  | Description                 | Type                  | Accepted values          | Default   |
| ---------- | --------------------------- | --------------------- | ------------------------ | --------- |
| **type**   | tag type                                    | [TagTypes](#tagtypes) | -                        | `default` |
| **invert** | filled variant: invert color and background | `boolean`              | -                        | `false`   |
| **light**  | light variant: tinted background            | `boolean`              | -                        | `false`   |
| **subtle** | subtle variant: no background               | `boolean`              | -                        | `false`   |
| ...        | native props                                | `HTMLAttributes`       | `'id', 'className', ...` | -         |

<Attributes.Title>TagTypes</Attributes.Title>

```ts
type TagTypes =
  | 'default'
  | 'secondary'
  | 'success'
  | 'warning'
  | 'error'
  | 'dark'
  | 'lite'
```

</Attributes>

export default Docs.withMeta(meta)
