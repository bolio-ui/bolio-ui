...
import { Select, Grid, Code, Modal, useModal, Button } from 'core'
...
The Select contain an additional Hooks, see subsection [useModal](/docs/hooks/use-modal) for details.
...
<Playground
  title="Divider"
  desc="You can use the divider of the `Select` with the property `divider`."
  scope={{ Select, Grid }}
  code={`
<Grid.Container gap={2}>
  <Grid>
...
<Playground
  ...
  scope={{ Select, Grid, useModal, Modal, Button, Code, Grid }}
  code={`
() => {
  ...
  return (
    <Grid.Container gap={2}>
      <Grid>
        <Button auto onClick={() => setVisible(true)}>Show more</Button>
        <Modal {...bindings}>
...
<Attributes edit="/content/docs/components/select.mdx">
