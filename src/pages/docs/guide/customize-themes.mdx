...
Not sure which colors to pick? Try them below. Bolio UI derives the shades it actually uses
(light, lighter and dark) from a primary and a secondary color, and gives you the code to drop
into `Themes.createFromLight`. For the full version, with a live preview of every `Button`
variant in light and dark, open the [Theme Generator](/theme-generator) page.

<ThemeGenerator compact />
