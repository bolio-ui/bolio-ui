...
| --------- | ----------------- |
| Most components, such as Card, Text, Badge, Tag, Grid, Tabs and Pagination | The root element |
| Input, Textarea, Checkbox, Radio and Toggle | The native `input` or `textarea` |
...
Select, Slider and Toggle can be used without a visible label. Name them with `aria-label`, or with `aria-labelledby` pointing at a text on the page:
